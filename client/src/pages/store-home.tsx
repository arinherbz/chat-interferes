        ...
        <section className="grid gap-6 overflow-hidden rounded-[2rem] border border-border bg-gradient-to-br from-white via-slate-50 to-slate-100 p-6 shadow-[0_30px_80px_rgba(15,23,42,0.08)] lg:grid-cols-[1.2fr_0.8fr] lg:p-10">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {(featured.length > 0 ? featured : products.slice(0, 4)).map((product) => (
              <Link key={product.id} href={`/store/products/${product.slug}`}>
                  ...
                  <CardContent className="space-y-3 p-4">
                    <div className="aspect-square overflow-hidden rounded-2xl bg-slate-100">
                      {product.imageUrl ? (
                        ...
                        <div className="flex h-full items-center justify-center text-slate-400">{product.brand || "Device"}</div>
                      )}
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">{product.condition || "New"}</p>
            ...
            {(flashDeals.length > 0 ? flashDeals : products.slice(0, 4)).map((product) => (
              <Card key={product.id} className="overflow-hidden">
                <CardContent className="space-y-3 p-5">
                  ...
                  <div>
                    <h3 className="font-medium">{product.name}</h3>
                    <p className="text-sm text-muted-foreground">{product.brand} {product.storage}</p>
                  </div>
